...
const ApplicationDates = () => {
  const [newMaleTime, setNewMaleTime] = useState([]);
  const [oldMaleTime, setOldMaleTime] = useState([]);
  const [oldFemaleTime, setOldFeMaleTime] = useState([]);
  const [newFemaleTime, setNewFemaleTime] = useState([]);

  useEffect(() => {
    axios
      .get("http://localhost:5000/timingNew/getNewFemales?")
      .then((response) => {
        // console.log(response);

        setNewFemaleTime(response.data.data.date);
      });
  });
  useEffect(() => {
    axios
      .get("http://localhost:5000/timingNew/getNewMales")
      .then((response) => {
        setNewMaleTime(response.data.data.date);
      });
  });
  return (
    <div className="main">
